        ...
        check_return_order: function (order) {
            ...
            for (var i = 0; i < lines.length; i++) {
                ...
                if (
                    ...
                    lines[i].returned_line_id &&
                    (qty_line > 0 || -1 * lines[i].quantity_returnable > qty_line)
                ) {
                    // Maximum quantity allowed exceeded
                    qty_incorrect_lines.push(
            ...
            if (qty_incorrect_lines.length + no_return_lines.length > 0) {
                ...
                if (qty_incorrect_lines.length > 0) {
                    error_message += "\n\n";
                    error_message +=
                        _t("* Invalid quantity line(s): ") +
                        qty_incorrect_lines.join(", ");
                }
                if (no_return_lines.length > 0) {
                    error_message += "\n\n";
                    error_message +=
                        _t("* Non-returnable line(s): ") + no_return_lines.join(", ");
                }
